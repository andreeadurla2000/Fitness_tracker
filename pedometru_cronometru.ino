...
float PragDetectiePasi = 11.45; //pragul de detectie pentru a incrementa numaratorul de pasi- determinat in urma analizei 
                                //inregistrarilor. Acest prag depinde de la persoana la persoana, si este determinat pentru mersul meu
...
const unsigned long secunda = 1000; //variabila ce va fi folosita impreuna cu millis()

//declararea pinilor pe care se conecteaza butoanele monostabile (4 butoane)
int butonS1 = 47;   
int butonS2 = 49;
int butonS3 = 51;
int butonS4 = 53;
...
bool cronometruPauza = false;//asigura functionalitatea de pauza a cronometrului
bool cronometruStop = false;
bool meniuCronometruPornit = false; // asigura contextualitatea meniului Cronometru, in functie de cum cronometrul este pornit sau nu
...
unsigned long momentPauza = 0; //determina momentul cand cronometrul a fost pus in pauza
...
void numarPasi() { 
    ...
    //atunci cand acceleratia rezultanta devine mai mica decat valoarea prag SI variabila PasActiv este adevarata (marcheaza ca se numara un pas)
...
void afiseazaPedometru(){
    while (digitalRead(butonS4) != LOW){ //intoarcerea in meniul principal se face apasand butonul S4 
      ...
      //numarul de pasi se va actualiza pe display 1 data pe secunda
      ...
      if (digitalRead(butonS3) == LOW) //atunci cand se doreste resetarea pedometrului, se apasa butonul S3
        NumarPasi=0;
   }
}
...
void afiseazaCronometru(){
  ...
  while(arataCronometru == true){ //intoarcerea in meniul principal se face apasand butonul S4 
